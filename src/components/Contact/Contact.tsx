...
if (EMAILJS_CONFIGURED) emailjs.init({ publicKey: PUBLIC_KEY! });
...
export default function Contact() {
  ...
  const title = lang === 'fr' ? 'LA CONVERSATION' : 'THE CONVERSATION';
  const bigStatement = lang === 'fr' ? 'Travaillons ensemble.' : "Let's build together.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const openMailto = () => {
      const link = `mailto:mguimfack@hotmail.com?subject=${encodeURIComponent(subject || `Contact from ${name}`)}&body=${encodeURIComponent(`From: ${name} (${email})\n\n${message}`)}`;
    ...
    if (EMAILJS_CONFIGURED) {
      try {
        await emailjs.send(SERVICE_ID!, TEMPLATE_ID!, {
          ...
          subject: subject || `Contact from ${name}`, message,
          to_name: 'Melvice', to_email: 'mguimfack@hotmail.com',
        });
        setStatus('sent');
  ...
  return (
    <section className="section contact-section" id="contact">
      <div className="container">
        <ChapterHeader number="07" title={title} italic />
        ...
        >
          <p className="contact-big">{bigStatement}</p>
                ...
                item.href ? (
                  ...
                  >
                    ...
                    <div className="link-text">
                      <span className="link-label">{t.contact.linkLabels[item.key]}</span>
                      <span className="link-value">{item.value}</span>
                    </div>
                    <span className="link-arrow">→</span>
                  </a>
                ) : (
                  <div key={item.key} className="contact-link contact-link-static">
                    <span className="link-icon">{item.icon}</span>
                    <div className="link-text">
                      <span className="link-label">{t.contact.linkLabels[item.key]}</span>
                      <span className="link-value">{item.value}</span>
          ...
          >
            <div className="form-row">
              <div className="form-group">
                ...
                <input id="name" name="name" type="text" placeholder={f.namePh}
                  value={formData.name} onChange={handleChange} required disabled={status === 'sending'} />
              </div>
              <div className="form-group">
                <label htmlFor="email">{f.emailLbl}</label>
                <input id="email" name="email" type="email" placeholder={f.emailPh}
                  value={formData.email} onChange={handleChange} required disabled={status === 'sending'} />
              </div>
            </div>

            <div className="form-group">
              <label htmlFor="subject">{f.subjectLbl}</label>
              <input id="subject" name="subject" type="text" placeholder={f.subjectPh}
                value={formData.subject} onChange={handleChange} disabled={status === 'sending'} />
            </div>

            <div className="form-group">
              <label htmlFor="message">{f.msgLbl}</label>
              <textarea id="message" name="message" rows={6} placeholder={f.msgPh}
                value={formData.message} onChange={handleChange} required disabled={status === 'sending'} />
            </div>
